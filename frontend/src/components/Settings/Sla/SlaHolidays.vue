<template>
    ...
    <NestedPopover>
      <template #target="{ open }">
        <Button
          ...
          :label="slaData.holiday_list || 'Select holiday list'"
        />
      </template>
      <template #body>
        <div
  ...
  <div class="mt-5">
    <div class="rounded-md border px-2 border-gray-300 text-sm">
      ...
      <hr v-if="slaData.working_hours?.length !== 0" />
      <SlaWorkDaysListItem
        v-for="(row, index) in slaData.working_hours"
        :key="index + row.workday + row.id"
        :row="row"
        :columns="columns"
        :isLast="index === slaData.working_hours.length - 1"
      />
      <div
        v-for="(row, index) in slaData.working_hours"
        :key="index + row.workday + row.id"
      ...
      >
        ...
        <hr v-if="!(index === slaData.working_hours.length - 1)" />
